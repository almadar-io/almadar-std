...
describe('useStepFlowReviewLogic', () => {
  ...
  describe('initial state', () => {
    it('should start in initial state', () => {
      ...
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });
      expect(result.current.state).toBe('loading');
    });

    it('should auto-dispatch INIT on mount', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      renderHook(() => useStepFlowReviewLogic(), { wrapper });
      await waitFor(() => {
  ...
  describe('event dispatch', () => {
    ...
    it('should dispatch StepItemsLoaded event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('StepItemsLoaded', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('StepItemsLoaded', { test: 'payload' }, expect.any(String));
    });

    it('should dispatch StepItemsLoadFailed event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('StepItemsLoadFailed', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('StepItemsLoadFailed', { test: 'payload' }, expect.any(String));
    });

    it('should dispatch StepItemsSaved event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('StepItemsSaved', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('StepItemsSaved', { test: 'payload' }, expect.any(String));
    });

    it('should dispatch StepItemsSaveFailed event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('StepItemsSaveFailed', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('StepItemsSaveFailed', { test: 'payload' }, expect.any(String));
    });
    ...
    it('should dispatch BACK event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('BACK', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('BACK', { test: 'payload' }, expect.any(String));
    });

    it('should dispatch REJECT event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('REJECT', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('REJECT', { test: 'payload' }, expect.any(String));
    });

    it('should dispatch ESCALATE event to server', async () => {
      mockSendEvent.mockResolvedValue({ success: true, newState: 'loading', data: {} });
      const { result } = renderHook(() => useStepFlowReviewLogic(), { wrapper });

      await act(async () => {
        await result.current.dispatch('ESCALATE', { test: 'payload' });
      });

      expect(mockSendEvent).toHaveBeenCalledWith('ESCALATE', { test: 'payload' }, expect.any(String));
    });
