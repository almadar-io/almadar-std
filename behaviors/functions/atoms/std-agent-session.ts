/**
 * std-agent-session — Session lifecycle atom for agent session management. Composes UI atoms (stdBrowse for session list, stdModal for fork label form) with an agent trait that handles agent/fork, agent/label, and agent/session-id.
 *
 ...
 *   node scripts/regenerate-std-ts.mjs --only=std-agent-session
 *
 * @level atom
...
const BEHAVIOR_PATH = 'std/behaviors/atoms/std-agent-session';
