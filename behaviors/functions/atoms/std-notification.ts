...
function buildTrait(c: NotificationConfig): Trait {
  ...
  // Visible state view: alert + toast-slot + violation-alert for comprehensive notification display
  const visibleView = {
    type: 'stack', direction: 'vertical', gap: 'lg',
    children: [
      {
      ...
      { type: 'toast-slot', position: 'top-right' },
      {
...
export function stdNotification(params: StdNotificationParams): OrbitalSchema {
