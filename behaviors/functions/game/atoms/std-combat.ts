/**
 * std-combat — std-combat as a Function
 *
 ...
 */
export type StdCombatEventKey = 'ATTACK' | 'COOLDOWN_END' | 'CombatantLoadFailed' | 'CombatantLoaded' | 'DEFEAT' | 'HIT' | 'INIT' | 'RESET';

/**
 * Payload shape for the `CombatantLoaded` event.
 */
export interface StdCombatCombatantLoadedPayload {
  id?: string;
  name?: string;
  description?: string;
  status?: string;
...
/**
 * Payload shape for the `CombatantLoadFailed` event.
 */
export interface StdCombatCombatantLoadFailedPayload {
  message?: string;
}
...
export interface StdCombatParams {
  ...
  events?: Partial<Record<StdCombatEventKey, string>>;
