/**
 * std-gameflow — std-gameflow as a Function
 *
...
const BEHAVIOR_PATH = 'std/behaviors/std-gameflow';
 ...
 */
export type StdGameflowEventKey = 'CLOSE' | 'GAME_OVER' | 'GameSessionLoadFailed' | 'GameSessionLoaded' | 'INIT' | 'NAVIGATE' | 'PAUSE' | 'RESTART' | 'RESUME' | 'START';

/**
 * Payload shape for the `GameSessionLoaded` event.
 */
export interface StdGameflowGameSessionLoadedPayload {
  id?: string;
  name?: string;
  description?: string;
  status?: string;
...
/**
 * Payload shape for the `GameSessionLoadFailed` event.
 */
export interface StdGameflowGameSessionLoadFailedPayload {
  message?: string;
}
...
export interface StdGameflowParams {
  ...
  events?: Partial<Record<StdGameflowEventKey, string>>;
