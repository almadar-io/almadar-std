/**
 * std-service-custom-bearer — std-service-custom-bearer — Bearer-token HTTP service. Bind via `uses CustomBearer from "std/behaviors/std-service-custom-bearer"` and configure `endpoint` / `method` / `body` / `token` at the call site. Pass `uiTrait: "@trait.YourBearerForm"` to inject your own request form; leave unset to render the atom's default test form (standalone surface).
 *
...
const BEHAVIOR_PATH = 'std/behaviors/std-service-custom-bearer';
 ...
 */
export type StdServiceCustomBearerEventKey = 'INIT' | 'SEND';

/**
 * Payload shape for the `SEND` event.
 */
export interface StdServiceCustomBearerSendPayload {
  source?: string;
}
...
/** Trait descriptor: `ServiceCustomBearer.traits.ServiceCustomBearerDefaultForm`. */
export function stdServiceCustomBearerServiceCustomBearerDefaultFormTrait(params: StdServiceCustomBearerParams): TraitReference {
  return makeTraitRef({
    from: BEHAVIOR_PATH,
    ref: `${ALIAS}.traits.ServiceCustomBearerDefaultForm`,
...
export function stdServiceCustomBearerServiceCustomBearerCustomBearerTrait(params: StdServiceCustomBearerParams): TraitReference {
  return makeTraitRef({
    from: BEHAVIOR_PATH,
...
export function stdServiceCustomBearer(params: StdServiceCustomBearerParams): OrbitalDefinition {
  ...
  return makeOrbitalWithUses({
    ...
    entity,
    traits: [
      stdServiceCustomBearerServiceCustomBearerDefaultFormTrait(params),
      stdServiceCustomBearerServiceCustomBearerCustomBearerTrait(params),
    ],
    pages: [
