/**
 * std-service-storage — std-service-storage — Object storage upload service. Bind via `uses Storage from "std/behaviors/std-service-storage"` and configure `bucket` / `acl` / `maxSize` / `allowedMimeTypes` at the call site. Pass `uiTrait: "@trait.YourUploadForm"` to inject your own picker; leave unset to render the atom's default file picker (standalone surface).
 *
 ...
 */
export type StdServiceStorageEventKey = 'INIT' | 'UPLOAD';

/**
 * Payload shape for the `UPLOAD` event.
 */
export interface StdServiceStorageUploadPayload {
  source?: string;
  file?: string;
}
...
/** Trait descriptor: `ServiceStorage.traits.ServiceStorageDefaultForm`. */
export function stdServiceStorageServiceStorageDefaultFormTrait(params: StdServiceStorageParams): TraitReference {
  return makeTraitRef({
    from: BEHAVIOR_PATH,
    ref: `${ALIAS}.traits.ServiceStorageDefaultForm`,
...
export function stdServiceStorageServiceStorageStorageTrait(params: StdServiceStorageParams): TraitReference {
  return makeTraitRef({
    from: BEHAVIOR_PATH,
...
export function stdServiceStorage(params: StdServiceStorageParams): OrbitalDefinition {
  ...
  return makeOrbitalWithUses({
    ...
    entity,
    traits: [
      stdServiceStorageServiceStorageDefaultFormTrait(params),
      stdServiceStorageServiceStorageStorageTrait(params),
    ],
    pages: [
