/**
 * ui-module-card — ModuleCard — a card summarizing one course module or lesson with progress and entry point. — UI factory (1:1 with the @almadar/ui module-card pattern).
 *
 * Autogenerated by almadar-sync std-ts from
 * packages/almadar-std/behaviors/registry/atoms/ui-module-card.orb.
 *
 ...
 *   npx almadar-sync std-ts --only=ui-module-card
 *
 * @level atom
...
const BEHAVIOR_PATH = 'std/behaviors/ui-module-card';
const ALIAS = 'UiModuleCard';
 ...
 */
export type StdUiModuleCardEventKey = 'INIT';
 ...
 */
export interface StdUiModuleCardParams {
  entityName: string;
  ...
  events?: Partial<Record<StdUiModuleCardEventKey, string>>;
  ...
  config?: StdUiModuleCardConfig;
  /** URL path override for the (first) page. */
  pagePath?: string;
}

/** Trait descriptor: `UiModuleCard.traits.ModuleCardRender`. */
export function stdUiModuleCardTrait(params: StdUiModuleCardParams): TraitReference {
  return makeTraitRef({
    from: BEHAVIOR_PATH,
    ref: `${ALIAS}.traits.ModuleCardRender`,
...
/** Page descriptor: `UiModuleCard.pages.ModuleCardPage`. */
export function stdUiModuleCardPage(params: StdUiModuleCardParams): PageRefObject {
  return makePageRef({
    from: BEHAVIOR_PATH,
    ref: `${ALIAS}.pages.ModuleCardPage`,
...
export function stdUiModuleCard(params: StdUiModuleCardParams): OrbitalDefinition {
  const entity: Entity = {
  ...
  return makeOrbitalWithUses({
    name: 'ModuleCardOrbital',
    uses: [{ from: BEHAVIOR_PATH, as: ALIAS }],
    entity,
    traits: [
      stdUiModuleCardTrait(params),
    ],
    pages: [
      stdUiModuleCardPage(params),
    ],
  });
}
