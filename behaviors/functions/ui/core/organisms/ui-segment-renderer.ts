/**
 * ui-segment-renderer — SegmentRenderer — UI factory (1:1 with the @almadar/ui segment-renderer pattern).
 *
 * Autogenerated by almadar-sync std-ts from
 * packages/almadar-std/behaviors/registry/organisms/ui-segment-renderer.orb.
 *
 ...
 *   npx almadar-sync std-ts --only=ui-segment-renderer
 *
 * @level organism
...
type _StdUiSegmentRendererListenTraitName = 'SegmentRendererRender';

/**
 * Tunable params for the SegmentRendererOrbital orbital.
 *
 * Canonical entity: SegmentRendererItem — overridable via
 ...
 */
export interface StdUiSegmentRendererSegmentRendererOrbitalParams {
  ...
  traitOverrides?: Partial<Record<
    'SegmentRendererRender',
    Pick<MakeTraitRefOpts, 'config' | 'linkedEntity' | 'events' | 'name' | 'emitsScope' | 'listens'>
  >>;
}

/** `'Alias.traits.TraitName'` literal union of every trait SegmentRendererOrbital's `uses[]` exports. */
type _StdUiSegmentRendererSegmentRendererOrbitalUsesRef = never;
  ...
  const built = makeOrbitalWithUses({
    ...
    traits: [
      rebindInlineTraitEntity({
        ...
        'id': asTraitId('trt_01KXG04SYNY0B326SBBN7KCRAW'),
        'linkedEntity': 'SegmentRendererItem',
        'linkedEntityId': asEntityId('ent_01KXG04SYN06XHEZN12SMFF8VV'),
    ...
    pages: [
      {
        'id': asPageId('pag_01KXG04SYQJTKCBEXDCD8332NJ'),
        'name': 'SegmentRendererPage',
        'path': '/segment-renderer',
        'traits': [
          {
            'ref': 'SegmentRendererRender',
            'refId': asTraitId('trt_01KXG04SYNY0B326SBBN7KCRAW'),
          },
        ],
      } satisfies Page,
    ],
...
/** Manifest — describes the params surface of stdUiSegmentRendererSegmentRendererOrbital. */
export const StdUiSegmentRendererSegmentRendererOrbitalManifest = {
  organism: 'ui-segment-renderer',
  orbitalName: 'SegmentRendererOrbital',
  paramFields: [
...
/** Typed guard — runtime validates StdUiSegmentRendererSegmentRendererOrbitalParams keys. */
export function isStdUiSegmentRendererSegmentRendererOrbitalParams(p: object): p is StdUiSegmentRendererSegmentRendererOrbitalParams {
  type _OverrideRecord = NonNullable<StdUiSegmentRendererSegmentRendererOrbitalParams['traitOverrides']>;
  const obj = p as { traitOverrides?: _OverrideRecord };
  if (obj.traitOverrides !== undefined) {
    if (typeof obj.traitOverrides !== "object" || obj.traitOverrides === null) return false;
    const allowed: readonly string[] = [
      ...StdUiSegmentRendererSegmentRendererOrbitalManifest.traitNames,
      ...StdUiSegmentRendererSegmentRendererOrbitalManifest.inlineTraitNames,
    ];
...
/**
 * Bundled params for ui-segment-renderer — one optional entry per orbital.
 * Each entry maps to its per-orbital factory above.
 */
export interface StdUiSegmentRendererParams {
  SegmentRenderer?: StdUiSegmentRendererSegmentRendererOrbitalParams;
}

/** Whole-organism descriptor (1 orbitals). Composes per-orbital factories. */
export function stdUiSegmentRenderer(params: StdUiSegmentRendererParams = {}): OrbitalDefinition[] {
  return [
    stdUiSegmentRendererSegmentRendererOrbital(params.SegmentRenderer ?? {}),
  ];
}
