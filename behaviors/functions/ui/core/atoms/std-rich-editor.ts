/**
 * std-rich-editor — std-rich-editor — wiki/CMS Notion-style block editor. Browses documents in `browsing`; opens the rich-block-editor on a selected row; persists block edits + publish status. `BlockEdit` is a rebindable single-row overlay editor: an organism rebinds it to its own entity (WikiPage, Note, …), passes the row's id + blocksJson via OPEN, and routes the emitted BLOCKS_SAVED into its own persistor — this atom never fetches or persists.
 *
      ...
      } satisfies Trait,
      {
        ...
        'config': {
          ...
          'title': {
            ...
            'description': 'Heading shown at the top of the block editor overlay.',
        ...
        'emits': [
          ...
          {
            'description': 'Indicates the block editor overlay is being dismissed.',
            'event': 'CLOSE',
        ...
        'stateMachine': {
          'events': [
            ...
            {
              'description': 'Requests the block editor overlay open for the given row. The caller supplies the row\'s id and current blocks — this trait never fetches them itself.',
              'key': 'OPEN',
              'name': 'Open',
              'payloadSchema': [
                {
