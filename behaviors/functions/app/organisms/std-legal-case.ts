/**
 * std-legal-case — Legal Case Management: matters, billable hours, and court deadlines for law firms and in-house counsel. Pick for litigation, transactional, and regulatory practice.
 *
 * Autogenerated by scripts/regenerate-std-ts.mjs from
 * packages/almadar-std/behaviors/registry/organisms/std-legal-case.orb.
 *
 * This is a thin descriptor layer over the canonical .orb. The exported
 * functions return TraitReference / PageRefObject / OrbitalDefinition
 * payloads that reference the sibling .orb via `uses:` + `ref:`. The
 * compiler's inline phase materialises the real trait at build time.
 *
 * Do not hand-edit — regenerate via
 *   node scripts/regenerate-std-ts.mjs --only=std-legal-case
 *
 * @level organism
 * @packageDocumentation
 */

import type { TraitReference, PageRefObject, OrbitalDefinition, Entity, EntityField, EntityPersistence, TraitConfig, TraitFieldRef, EntityRow, SExpr, TraitEventListener, Trait, StateMachine } from '@almadar/core/types';
import type { MakeTraitRefOpts } from '@almadar/core/builders';
import { makeTraitRef, makePageRef, makeOrbitalWithUses } from '@almadar/core/builders';

const BEHAVIOR_PATH = 'std/behaviors/std-legal-case';
const ALIAS = 'LegalCase';

/**
 * Typed call-site config block for this trait — every
 * field maps to a `config { ... }` entry in the source
 * .lolo. The agent fills these to specialise the trait
 * without modifying its state-machine topology.
 */
export interface StdLegalCaseConfig {
  notifications?: TraitConfig;
  navItems?: TraitConfig;
}

/**
 * Tunable params for the MatterOrbital orbital.
 *
 * Canonical entity: Matter (locked — not overridable).
 * The factory hardcodes `linkedEntity` to the canonical entity on
 * every trait/page; renaming the entity would desync those references.
 *
 * Override surface (Phase 9):
 *   fields            — extra entity fields (appended)
 *   pagePath          — first-page URL override
 *   persistence       — entity persistence mode
 *   traitOverrides    — Zone 1: per-imported-trait override surface
 *                       (config / events / effects / listens / emitsScope /
 *                       linkedEntity). Atom owns topology.
 *   interactionStates — Zone 2: inline-trait state-machine splice
 *                       (states / events / transitions). Factory
 *                       owns topology.
 */
export interface StdLegalCaseMatterOrbitalParams {
  /** Extra fields appended to the canonical entity. */
  fields?: EntityField[];
  /** URL path override for the orbital's first page. */
  pagePath?: string;
  /** Override the canonical entity persistence mode. */
  persistence?: EntityPersistence;
  /**
   * Zone 1 — per-imported-trait override surface. Keyed on each
   * imported trait's local `name`. The atom owns state-machine
   * topology (states/events/transitions); these fields rewrite the
   * caller-side surface only.
   */
  traitOverrides?: Partial<Record<
    'MatterAppLayout' | 'MatterSearch' | 'MatterFilter' | 'MatterStats' | 'MatterGraphs' | 'MatterBrowseList' | 'MatterCreate' | 'MatterEdit' | 'MatterView' | 'MatterDelete',
    Pick<MakeTraitRefOpts, 'config' | 'events' | 'effects' | 'listens' | 'emitsScope' | 'linkedEntity'>
  >>;
  /**
   * Zone 2 — inline-trait state-machine splice. Keyed on each inline
   * (factory-authored, no `ref:`) trait's `name`. Merges into the
   * matching inline trait's `stateMachine` block. Factory owns topology;
   * splice inserts caller-named states/events/transitions.
   */
  interactionStates?: Partial<Record<
    'MatterCatalog' | 'MatterPersistor',
    Partial<Pick<NonNullable<Trait['stateMachine']>, 'states' | 'events' | 'transitions'>>
  >>;
}

/** Per-orbital factory: builds the MatterOrbital orbital with consumer params. */
export function stdLegalCaseMatterOrbital(params: StdLegalCaseMatterOrbitalParams = {}): OrbitalDefinition {
  const canonicalName = 'Matter';
  const built = makeOrbitalWithUses({
    name: 'MatterOrbital',
    uses: [
      {
        'from': 'std/behaviors/std-app-layout',
        'as': 'AppShell',
      },
      {
        'from': 'std/behaviors/std-modal',
        'as': 'Modal',
      },
      {
        'from': 'std/behaviors/std-confirmation',
        'as': 'Confirmation',
      },
      {
        'from': 'std/behaviors/std-search',
        'as': 'Search',
      },
      {
        'from': 'std/behaviors/std-filter',
        'as': 'Filter',
      },
      {
        'from': 'std/behaviors/std-stats',
        'as': 'Stats',
      },
      {
        'from': 'std/behaviors/std-graphs',
        'as': 'Graphs',
      },
      {
        'from': 'std/behaviors/std-browse',
        'as': 'Browse',
      },
      {
        'from': 'std/behaviors/std-matter',
        'as': 'Matter',
      },
      {
        'from': 'std/behaviors/std-billable-hour',
        'as': 'BillableHour',
      },
      {
        'from': 'std/behaviors/std-court-deadline',
        'as': 'CourtDeadline',
      },
    ],
    entity: {
      name: canonicalName,
      collection: 'matters',
      persistence: params.persistence ?? 'persistent',
      fields: [
        {
          'name': 'id',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'matterNumber',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'title',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'clientName',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'practiceArea',
          'type': 'string',
          'default': 'other',
          'values': [
            'litigation',
            'transactional',
            'regulatory',
            'ip',
            'immigration',
            'family',
            'other',
          ],
        },
        {
          'name': 'leadAttorney',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'status',
          'type': 'string',
          'default': 'open',
          'values': [
            'open',
            'active',
            'on_hold',
            'closed',
            'archived',
          ],
        },
        {
          'name': 'openedAt',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'closedAt',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'notes',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'pendingId',
          'type': 'string',
          'default': '',
        },
        ...(params.fields ?? []),
      ],
    } as Entity,
    traits: [
      makeTraitRef({
        'ref': 'AppShell.traits.AppLayout',
        'name': 'MatterAppLayout',
        'config': {
          'appName': 'Legal Case',
          'contentTrait': '@trait.MatterCatalog',
          'searchEvent': 'MATTER_SEARCH',
          'notifications': [],
          'notificationClickEvent': 'MATTER_NOTIFICATIONS_OPEN',
          'navItems': [
            {
              'icon': 'scale',
              'href': '/matters',
              'label': 'Matters',
            },
            {
              'label': 'Time',
              'href': '/timesheet',
              'icon': 'clock',
            },
            {
              'icon': 'calendar',
              'label': 'Deadlines',
              'href': '/deadlines',
            },
          ],
        },
        'events': {
          'SEARCH': 'MATTER_SEARCH',
          'NOTIFY_CLICK': 'MATTER_NOTIFICATIONS_OPEN',
        },
      }),
      {
        'name': 'MatterCatalog',
        'category': 'interaction',
        'emits': [
          {
            'event': 'CREATE',
            'scope': 'external',
            'payloadSchema': [
              {
                'name': 'source',
                'type': 'string',
              },
            ],
          },
        ],
        'listens': [
          {
            'event': 'MATTER_SEARCH',
            'triggers': 'MATTER_SEARCH',
            'source': {
              'kind': 'trait',
              'trait': 'MatterAppLayout',
            },
          },
          {
            'event': 'MATTER_NOTIFICATIONS_OPEN',
            'triggers': 'MATTER_NOTIFICATIONS_OPEN',
            'source': {
              'kind': 'trait',
              'trait': 'MatterAppLayout',
            },
          },
        ],
        'stateMachine': {
          'states': [
            {
              'name': 'composing',
              'isInitial': true,
            },
          ],
          'events': [
            {
              'key': 'INIT',
              'name': 'Initialize',
            },
            {
              'key': 'MATTER_SEARCH',
              'name': 'Matter Search',
              'payloadSchema': [
                {
                  'name': 'value',
                  'type': 'string',
                },
              ],
            },
            {
              'key': 'MATTER_NOTIFICATIONS_OPEN',
              'name': 'Matter Notifications Open',
              'payloadSchema': [
                {
                  'name': 'id',
                  'type': 'string',
                },
              ],
            },
            {
              'key': 'CREATE',
              'name': 'Create',
            },
          ],
          'transitions': [
            {
              'from': 'composing',
              'to': 'composing',
              'event': 'INIT',
              'effects': [
                [
                  'render-ui',
                  'main',
                  {
                    'direction': 'vertical',
                    'gap': 'lg',
                    'children': [
                      {
                        'direction': 'horizontal',
                        'gap': 'md',
                        'type': 'stack',
                        'justify': 'between',
                        'align': 'center',
                        'children': [
                          {
                            'children': [
                              {
                                'type': 'icon',
                                'name': 'scale',
                              },
                              {
                                'type': 'typography',
                                'variant': 'h2',
                                'content': 'Matters',
                              },
                            ],
                            'gap': 'sm',
                            'direction': 'horizontal',
                            'type': 'stack',
                            'align': 'center',
                          },
                          {
                            'children': [
                              {
                                'variant': 'primary',
                                'action': 'CREATE',
                                'label': 'New Matter',
                                'icon': 'plus',
                                'type': 'button',
                              },
                            ],
                            'type': 'stack',
                            'direction': 'horizontal',
                            'gap': 'sm',
                          },
                        ],
                      },
                      {
                        'type': 'divider',
                      },
                      {
                        'type': 'stack',
                        'gap': 'md',
                        'align': 'center',
                        'direction': 'horizontal',
                        'children': [
                          '@trait.MatterSearch',
                          '@trait.MatterFilter',
                        ],
                      },
                      '@trait.MatterStats',
                      '@trait.MatterGraphs',
                      {
                        'type': 'divider',
                      },
                      '@trait.MatterBrowseList',
                    ],
                    'type': 'stack',
                  },
                ],
              ],
            },
            {
              'from': 'composing',
              'to': 'composing',
              'event': 'MATTER_SEARCH',
            },
            {
              'from': 'composing',
              'to': 'composing',
              'event': 'MATTER_NOTIFICATIONS_OPEN',
              'effects': [
                [
                  'render-ui',
                  'main',
                  {
                    'children': [
                      {
                        'type': 'icon',
                        'name': 'bell',
                      },
                      {
                        'type': 'typography',
                        'variant': 'h3',
                        'content': 'No notifications',
                      },
                      {
                        'content': 'You\'re all caught up.',
                        'variant': 'caption',
                        'color': 'muted',
                        'type': 'typography',
                      },
                      {
                        'action': 'INIT',
                        'label': 'Back to matters',
                        'variant': 'ghost',
                        'type': 'button',
                      },
                    ],
                    'align': 'center',
                    'className': 'py-8',
                    'direction': 'vertical',
                    'gap': 'md',
                    'type': 'stack',
                  },
                ],
              ],
            },
          ],
        },
        'scope': 'instance',
      } as never,
      makeTraitRef({
        'ref': 'Search.traits.SearchResultSearch',
        'name': 'MatterSearch',
        'config': {
          'event': 'MATTER_SEARCH',
          'placeholder': 'Search matters…',
        },
      }),
      makeTraitRef({
        'ref': 'Filter.traits.FilterTargetFilter',
        'name': 'MatterFilter',
        'config': {
          'event': 'MATTER_FILTER',
          'filters': [
            {
              'field': 'practiceArea',
              'filterType': 'select',
              'label': 'Practice',
              'options': [
                'litigation',
                'transactional',
                'regulatory',
                'ip',
                'immigration',
                'family',
                'other',
              ],
            },
            {
              'field': 'status',
              'filterType': 'select',
              'options': [
                'open',
                'active',
                'on_hold',
                'closed',
                'archived',
              ],
              'label': 'Status',
            },
          ],
        },
      }),
      makeTraitRef({
        'ref': 'Stats.traits.StatsItemStats',
        'name': 'MatterStats',
        'config': {
          'title': 'Matters',
          'metrics': [
            {
              'format': 'number',
              'label': 'Total',
              'aggregation': 'count',
              'variant': 'primary',
              'icon': 'scale',
            },
            {
              'icon': 'circle',
              'aggregation': 'count',
              'filter': [
                'fn',
                'row',
                [
                  '=',
                  '@row.status',
                  'open',
                ],
              ],
              'format': 'number',
              'label': 'Open',
              'variant': 'warning',
            },
            {
              'variant': 'success',
              'filter': [
                'fn',
                'row',
                [
                  '=',
                  '@row.status',
                  'active',
                ],
              ],
              'format': 'number',
              'aggregation': 'count',
              'label': 'Active',
              'icon': 'check-circle',
            },
            {
              'label': 'Closed',
              'variant': 'info',
              'icon': 'archive',
              'filter': [
                'fn',
                'row',
                [
                  '=',
                  '@row.status',
                  'closed',
                ],
              ],
              'format': 'number',
              'aggregation': 'count',
            },
          ],
        },
        'listens': [
          {
            'event': 'BrowseItemLoaded',
            'triggers': 'ITEMS_LOADED',
            'source': {
              'kind': 'trait',
              'trait': 'MatterBrowseList',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Graphs.traits.GraphItemGraph',
        'name': 'MatterGraphs',
        'config': {
          'categoryField': 'practiceArea',
          'height': 240,
          'title': 'Matters by Practice Area',
          'aggregation': 'count',
          'showLegend': false,
          'chartType': 'bar',
          'subtitle': 'Caseload composition across practice areas',
        },
        'listens': [
          {
            'event': 'BrowseItemLoaded',
            'triggers': 'ITEMS_LOADED',
            'source': {
              'kind': 'trait',
              'trait': 'MatterBrowseList',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Browse.traits.BrowseItemBrowse',
        'name': 'MatterBrowseList',
        'linkedEntity': 'Matter',
        'config': {
          'fields': [
            {
              'variant': 'caption',
              'icon': 'hash',
              'name': 'matterNumber',
            },
            {
              'name': 'title',
              'variant': 'h3',
            },
            {
              'variant': 'body',
              'name': 'clientName',
            },
            {
              'name': 'practiceArea',
              'variant': 'badge',
            },
            {
              'name': 'leadAttorney',
              'variant': 'caption',
            },
            {
              'name': 'status',
              'variant': 'badge',
            },
          ],
          'cols': 1,
          'itemActions': [
            {
              'label': 'View',
              'event': 'VIEW',
              'variant': 'ghost',
            },
            {
              'variant': 'ghost',
              'event': 'EDIT',
              'label': 'Edit',
            },
            {
              'event': 'DELETE',
              'label': 'Delete',
              'variant': 'danger',
            },
          ],
          'gap': 'sm',
        },
        'listens': [
          {
            'event': 'SEARCH',
            'triggers': 'REFETCH_QUERY',
            'source': {
              'kind': 'trait',
              'trait': 'MatterSearch',
            },
          },
          {
            'event': 'FILTER',
            'triggers': 'REFETCH_FILTER',
            'source': {
              'kind': 'trait',
              'trait': 'MatterFilter',
            },
          },
          {
            'event': 'MATTER_CREATED',
            'triggers': 'INIT',
            'source': {
              'kind': 'trait',
              'trait': 'MatterPersistor',
            },
          },
          {
            'event': 'MATTER_UPDATED',
            'triggers': 'INIT',
            'source': {
              'kind': 'trait',
              'trait': 'MatterPersistor',
            },
          },
          {
            'event': 'MATTER_DELETED',
            'triggers': 'INIT',
            'source': {
              'kind': 'trait',
              'trait': 'MatterPersistor',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Modal.traits.ModalRecordModal',
        'name': 'MatterCreate',
        'linkedEntity': 'Matter',
        'config': {
          'fields': [
            'matterNumber',
            'title',
            'clientName',
            'practiceArea',
            'leadAttorney',
            'status',
            'openedAt',
            'notes',
          ],
          'title': 'New Matter',
          'mode': 'create',
          'icon': 'plus-circle',
        },
        'events': {
          'OPEN': 'CREATE',
        },
        'listens': [
          {
            'event': 'CREATE',
            'triggers': 'CREATE',
            'source': {
              'kind': 'trait',
              'trait': 'MatterCatalog',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Modal.traits.ModalRecordModal',
        'name': 'MatterEdit',
        'linkedEntity': 'Matter',
        'config': {
          'title': 'Edit Matter',
          'mode': 'edit',
          'icon': 'edit',
          'fields': [
            'matterNumber',
            'title',
            'clientName',
            'practiceArea',
            'leadAttorney',
            'status',
            'openedAt',
            'closedAt',
            'notes',
          ],
        },
        'events': {
          'OPEN': 'EDIT',
        },
        'listens': [
          {
            'event': 'EDIT',
            'triggers': 'EDIT',
            'source': {
              'kind': 'trait',
              'trait': 'MatterBrowseList',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Modal.traits.ModalRecordModal',
        'name': 'MatterView',
        'linkedEntity': 'Matter',
        'config': {
          'icon': 'eye',
          'title': 'View Matter',
          'mode': 'edit',
          'fields': [
            'matterNumber',
            'title',
            'clientName',
            'practiceArea',
            'leadAttorney',
            'status',
            'openedAt',
            'closedAt',
            'notes',
          ],
        },
        'events': {
          'OPEN': 'VIEW',
        },
        'listens': [
          {
            'event': 'VIEW',
            'triggers': 'VIEW',
            'source': {
              'kind': 'trait',
              'trait': 'MatterBrowseList',
            },
          },
        ],
      }),
      makeTraitRef({
        'ref': 'Confirmation.traits.ConfirmActionConfirmation',
        'name': 'MatterDelete',
        'linkedEntity': 'Matter',
        'config': {
          'title': 'Delete Matter',
          'confirmLabel': 'Delete',
          'icon': 'alert-triangle',
          'alertMessage': 'This action cannot be undone.',
        },
        'events': {
          'REQUEST': 'DELETE',
          'CONFIRM': 'CONFIRM_DELETE',
        },
        'listens': [
          {
            'event': 'DELETE',
            'triggers': 'DELETE',
            'source': {
              'kind': 'trait',
              'trait': 'MatterBrowseList',
            },
          },
        ],
      }),
      {
        'name': 'MatterPersistor',
        'category': 'lifecycle',
        'linkedEntity': 'Matter',
        'emits': [
          {
            'event': 'MATTER_CREATED',
            'scope': 'external',
            'payloadSchema': [
              {
                'name': 'id',
                'type': 'string',
              },
            ],
          },
          {
            'event': 'MATTER_UPDATED',
            'scope': 'external',
            'payloadSchema': [
              {
                'name': 'id',
                'type': 'string',
              },
            ],
          },
          {
            'event': 'MATTER_DELETED',
            'scope': 'external',
            'payloadSchema': [
              {
                'name': 'id',
                'type': 'string',
              },
            ],
          },
        ],
        'listens': [
          {
            'event': 'SAVE',
            'triggers': 'DO_CREATE',
            'source': {
              'kind': 'trait',
              'trait': 'MatterCreate',
            },
          },
          {
            'event': 'SAVE',
            'triggers': 'DO_UPDATE',
            'source': {
              'kind': 'trait',
              'trait': 'MatterEdit',
            },
          },
          {
            'event': 'CONFIRM_DELETE',
            'triggers': 'DO_DELETE',
            'source': {
              'kind': 'trait',
              'trait': 'MatterDelete',
            },
          },
        ],
        'stateMachine': {
          'states': [
            {
              'name': 'idle',
              'isInitial': true,
            },
          ],
          'events': [
            {
              'key': 'INIT',
              'name': 'Initialize',
            },
            {
              'key': 'DO_CREATE',
              'name': 'Do Create',
              'payloadSchema': [
                {
                  'name': 'data',
                  'type': 'object',
                  'required': true,
                },
              ],
            },
            {
              'key': 'DO_UPDATE',
              'name': 'Do Update',
              'payloadSchema': [
                {
                  'name': 'data',
                  'type': 'object',
                  'required': true,
                },
              ],
            },
            {
              'key': 'DO_DELETE',
              'name': 'Do Delete',
              'payloadSchema': [
                {
                  'name': 'id',
                  'type': 'string',
                },
              ],
            },
            {
              'key': 'MATTER_CREATED',
              'name': 'Matter Created',
            },
            {
              'key': 'MATTER_UPDATED',
              'name': 'Matter Updated',
            },
            {
              'key': 'MATTER_DELETED',
              'name': 'Matter Deleted',
            },
          ],
          'transitions': [
            {
              'from': 'idle',
              'to': 'idle',
              'event': 'INIT',
            },
            {
              'from': 'idle',
              'to': 'idle',
              'event': 'DO_CREATE',
              'effects': [
                [
                  'persist',
                  'create',
                  'Matter',
                  '@payload.data',
                  {
                    'emit': {
                      'success': 'MATTER_CREATED',
                    },
                  },
                ],
              ],
            },
            {
              'from': 'idle',
              'to': 'idle',
              'event': 'DO_UPDATE',
              'effects': [
                [
                  'persist',
                  'update',
                  'Matter',
                  '@payload.data',
                  {
                    'emit': {
                      'success': 'MATTER_UPDATED',
                    },
                  },
                ],
              ],
            },
            {
              'from': 'idle',
              'to': 'idle',
              'event': 'DO_DELETE',
              'effects': [
                [
                  'persist',
                  'delete',
                  'Matter',
                  '@payload.id',
                  {
                    'emit': {
                      'success': 'MATTER_DELETED',
                    },
                  },
                ],
              ],
            },
          ],
        },
        'scope': 'instance',
      } as never,
    ],
    pages: [
      {
        'name': 'MattersPage',
        'path': '/matters',
        'traits': [
          {
            'ref': 'MatterAppLayout',
          },
          {
            'ref': 'MatterCatalog',
          },
          {
            'ref': 'MatterSearch',
          },
          {
            'ref': 'MatterFilter',
          },
          {
            'ref': 'MatterStats',
          },
          {
            'ref': 'MatterGraphs',
          },
          {
            'ref': 'MatterBrowseList',
          },
          {
            'ref': 'MatterCreate',
          },
          {
            'ref': 'MatterEdit',
          },
          {
            'ref': 'MatterView',
          },
          {
            'ref': 'MatterDelete',
          },
          {
            'ref': 'MatterPersistor',
          },
        ],
      } as never,
    ],
  });
  type _OrbTrait = OrbitalDefinition["traits"][number];
  type _OrbPage = NonNullable<OrbitalDefinition["pages"]>[number];
  type _RefOverride = Pick<MakeTraitRefOpts, "config" | "events" | "effects" | "listens" | "emitsScope" | "linkedEntity">;
  type _InlineSplice = Partial<Pick<NonNullable<Trait["stateMachine"]>, "states" | "events" | "transitions">>;
  // Zone 1: per-imported-trait override surface, keyed by trait `name`.
  // After the `ref:`-string filter, the trait variant is TraitReference,
  // whose surface matches the override surface 1:1.
  if (built.traits && params.traitOverrides !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as TraitReference;
      if (typeof tr.ref !== "string" || typeof tr.name !== "string") return t;
      const overrides = params.traitOverrides as Record<string, _RefOverride | undefined> | undefined;
      const override = overrides?.[tr.name];
      if (!override) return t;
      const merged: TraitReference = { ...tr };
      if (override.config !== undefined) merged.config = { ...(tr.config ?? {}), ...override.config };
      if (override.events !== undefined) merged.events = { ...(tr.events ?? {}), ...override.events };
      if (override.effects !== undefined) merged.effects = { ...(tr.effects ?? {}), ...override.effects };
      if (override.listens !== undefined) merged.listens = override.listens;
      if (override.emitsScope !== undefined) merged.emitsScope = override.emitsScope;
      if (override.linkedEntity !== undefined) merged.linkedEntity = override.linkedEntity;
      return merged;
    });
  }
  // Zone 2: inline-trait state-machine splice, keyed by inline trait `name`.
  // After the `ref:`-not-set filter, the trait variant is Trait, whose
  // stateMachine block is what we splice into.
  if (built.traits && params.interactionStates !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as Trait & { ref?: string };
      if (typeof tr.ref === "string" || typeof tr.name !== "string") return t;
      const splices = params.interactionStates as Record<string, _InlineSplice | undefined> | undefined;
      const splice = splices?.[tr.name];
      if (!splice || !tr.stateMachine) return t;
      const sm = tr.stateMachine;
      const mergedSm: StateMachine = {
        ...sm,
        ...(splice.states !== undefined ? { states: [...(sm.states ?? []), ...splice.states] } : {}),
        ...(splice.events !== undefined ? { events: [...(sm.events ?? []), ...splice.events] } : {}),
        ...(splice.transitions !== undefined ? { transitions: [...(sm.transitions ?? []), ...splice.transitions] } : {}),
      };
      const mergedTrait: Trait = { ...tr, stateMachine: mergedSm };
      return mergedTrait;
    });
  }
  if (built.pages && params.pagePath !== undefined) {
    built.pages = (built.pages as _OrbPage[]).map((p, idx) => {
      if (!p || typeof p !== "object") return p;
      if (idx !== 0) return p;
      const out = { ...p } as _OrbPage & { path?: string };
      out.path = params.pagePath;
      return out;
    });
  }
  return built;
}

/** Phase 9 manifest — describes the params surface of stdLegalCaseMatterOrbital. */
export const StdLegalCaseMatterOrbitalManifest = {
  organism: 'std-legal-case',
  orbitalName: 'MatterOrbital',
  paramFields: [
    { name: 'fields', type: 'EntityField[]', description: 'Extra fields appended to the canonical entity.' },
    { name: 'pagePath', type: 'string', description: 'URL override for the orbital first page.' },
    { name: 'persistence', type: "'persistent' | 'runtime' | 'singleton' | 'instance' | 'local'", description: 'Override the canonical entity persistence mode.' },
    { name: 'traitOverrides', type: 'Partial<Record<TraitName, MakeTraitRefOpts>>', description: 'Zone 1 — per-imported-trait override surface (config / events / effects / listens / emitsScope / linkedEntity). Atom owns topology.' },
    { name: 'interactionStates', type: 'Partial<Record<InlineTraitName, StateMachineSplice>>', description: 'Zone 2 — inline-trait state-machine splice (states / events / transitions). Factory owns topology.' },
  ] as const,
  traitNames: [
    'MatterAppLayout',
    'MatterSearch',
    'MatterFilter',
    'MatterStats',
    'MatterGraphs',
    'MatterBrowseList',
    'MatterCreate',
    'MatterEdit',
    'MatterView',
    'MatterDelete',
  ] as const,
  inlineTraitNames: [
    'MatterCatalog',
    'MatterPersistor',
  ] as const,
};

/** Phase 9 typed guard — runtime validates StdLegalCaseMatterOrbitalParams keys. */
export function isStdLegalCaseMatterOrbitalParams(p: object): p is StdLegalCaseMatterOrbitalParams {
  type _OverrideRecord = NonNullable<StdLegalCaseMatterOrbitalParams['traitOverrides']>;
  type _SpliceRecord = NonNullable<StdLegalCaseMatterOrbitalParams['interactionStates']>;
  const obj = p as { traitOverrides?: _OverrideRecord; interactionStates?: _SpliceRecord };
  if (obj.traitOverrides !== undefined) {
    if (typeof obj.traitOverrides !== "object" || obj.traitOverrides === null) return false;
    const allowed: readonly string[] = StdLegalCaseMatterOrbitalManifest.traitNames;
    for (const k of Object.keys(obj.traitOverrides)) {
      if (!allowed.includes(k)) return false;
    }
  }
  if (obj.interactionStates !== undefined) {
    if (typeof obj.interactionStates !== "object" || obj.interactionStates === null) return false;
    const allowed: readonly string[] = StdLegalCaseMatterOrbitalManifest.inlineTraitNames;
    for (const k of Object.keys(obj.interactionStates)) {
      if (!allowed.includes(k)) return false;
    }
  }
  return true;
}

/**
 * Tunable params for the BillableHourPanelOrbital orbital.
 *
 * Canonical entity: BillableHour (locked — not overridable).
 * The factory hardcodes `linkedEntity` to the canonical entity on
 * every trait/page; renaming the entity would desync those references.
 *
 * Override surface (Phase 9):
 *   fields            — extra entity fields (appended)
 *   pagePath          — first-page URL override
 *   persistence       — entity persistence mode
 *   traitOverrides    — Zone 1: per-imported-trait override surface
 *                       (config / events / effects / listens / emitsScope /
 *                       linkedEntity). Atom owns topology.
 *   interactionStates — Zone 2: inline-trait state-machine splice
 *                       (states / events / transitions). Factory
 *                       owns topology.
 */
export interface StdLegalCaseBillableHourPanelOrbitalParams {
  /** Extra fields appended to the canonical entity. */
  fields?: EntityField[];
  /** URL path override for the orbital's first page. */
  pagePath?: string;
  /** Override the canonical entity persistence mode. */
  persistence?: EntityPersistence;
  /**
   * Zone 1 — per-imported-trait override surface. Keyed on each
   * imported trait's local `name`. The atom owns state-machine
   * topology (states/events/transitions); these fields rewrite the
   * caller-side surface only.
   */
  traitOverrides?: Partial<Record<
    'BillableHourAppLayout' | 'BillableHourTimesheetView',
    Pick<MakeTraitRefOpts, 'config' | 'events' | 'effects' | 'listens' | 'emitsScope' | 'linkedEntity'>
  >>;
  /**
   * Zone 2 — inline-trait state-machine splice. Keyed on each inline
   * (factory-authored, no `ref:`) trait's `name`. Merges into the
   * matching inline trait's `stateMachine` block. Factory owns topology;
   * splice inserts caller-named states/events/transitions.
   */
  interactionStates?: Partial<Record<
    'BillableHourPanel',
    Partial<Pick<NonNullable<Trait['stateMachine']>, 'states' | 'events' | 'transitions'>>
  >>;
}

/** Per-orbital factory: builds the BillableHourPanelOrbital orbital with consumer params. */
export function stdLegalCaseBillableHourPanelOrbital(params: StdLegalCaseBillableHourPanelOrbitalParams = {}): OrbitalDefinition {
  const canonicalName = 'BillableHour';
  const built = makeOrbitalWithUses({
    name: 'BillableHourPanelOrbital',
    uses: [
      {
        'from': 'std/behaviors/std-app-layout',
        'as': 'AppShell',
      },
      {
        'from': 'std/behaviors/std-billable-hour',
        'as': 'BillableHour',
      },
    ],
    entity: {
      name: canonicalName,
      collection: 'billablehours',
      persistence: params.persistence ?? 'persistent',
      fields: [
        {
          'name': 'id',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'workerId',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'workerName',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'matterId',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'projectId',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'workDate',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'hours',
          'type': 'number',
          'default': 0,
        },
        {
          'name': 'rate',
          'type': 'number',
          'default': 0,
        },
        {
          'name': 'description',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'isBillable',
          'type': 'boolean',
          'default': true,
        },
        {
          'name': 'invoiceStatus',
          'type': 'string',
          'default': 'unbilled',
          'values': [
            'unbilled',
            'invoiced',
            'paid',
            'written_off',
          ],
        },
        ...(params.fields ?? []),
      ],
    } as Entity,
    traits: [
      makeTraitRef({
        'ref': 'AppShell.traits.AppLayout',
        'name': 'BillableHourAppLayout',
        'config': {
          'notifications': [],
          'notificationClickEvent': 'BILLABLE_HOUR_NOTIFICATIONS_OPEN',
          'navItems': [
            {
              'icon': 'scale',
              'label': 'Matters',
              'href': '/matters',
            },
            {
              'href': '/timesheet',
              'label': 'Time',
              'icon': 'clock',
            },
            {
              'icon': 'calendar',
              'href': '/deadlines',
              'label': 'Deadlines',
            },
          ],
          'appName': 'Legal Case',
          'searchEvent': 'BILLABLE_HOUR_SEARCH',
          'contentTrait': '@trait.BillableHourPanel',
        },
        'events': {
          'SEARCH': 'BILLABLE_HOUR_SEARCH',
          'NOTIFY_CLICK': 'BILLABLE_HOUR_NOTIFICATIONS_OPEN',
        },
      }),
      {
        'name': 'BillableHourPanel',
        'category': 'interaction',
        'stateMachine': {
          'states': [
            {
              'name': 'composing',
              'isInitial': true,
            },
          ],
          'events': [
            {
              'key': 'INIT',
              'name': 'Initialize',
            },
          ],
          'transitions': [
            {
              'from': 'composing',
              'to': 'composing',
              'event': 'INIT',
              'effects': [
                [
                  'render-ui',
                  'main',
                  {
                    'gap': 'lg',
                    'children': [
                      {
                        'gap': 'sm',
                        'type': 'stack',
                        'direction': 'horizontal',
                        'children': [
                          {
                            'name': 'clock',
                            'type': 'icon',
                          },
                          {
                            'variant': 'h2',
                            'content': 'Time Entries',
                            'type': 'typography',
                          },
                        ],
                        'align': 'center',
                      },
                      {
                        'type': 'divider',
                      },
                      '@trait.BillableHourTimesheetView',
                    ],
                    'className': 'max-w-6xl mx-auto w-full p-4',
                    'type': 'stack',
                    'direction': 'vertical',
                  },
                ],
              ],
            },
          ],
        },
        'scope': 'instance',
      } as never,
      makeTraitRef({
        'ref': 'BillableHour.traits.BillableHourTimesheet',
        'name': 'BillableHourTimesheetView',
        'config': {
          'title': 'Billable Hours',
        },
      }),
    ],
    pages: [
      {
        'name': 'TimesheetPage',
        'path': '/timesheet',
        'traits': [
          {
            'ref': 'BillableHourAppLayout',
          },
          {
            'ref': 'BillableHourPanel',
          },
          {
            'ref': 'BillableHourTimesheetView',
          },
        ],
      } as never,
    ],
  });
  type _OrbTrait = OrbitalDefinition["traits"][number];
  type _OrbPage = NonNullable<OrbitalDefinition["pages"]>[number];
  type _RefOverride = Pick<MakeTraitRefOpts, "config" | "events" | "effects" | "listens" | "emitsScope" | "linkedEntity">;
  type _InlineSplice = Partial<Pick<NonNullable<Trait["stateMachine"]>, "states" | "events" | "transitions">>;
  // Zone 1: per-imported-trait override surface, keyed by trait `name`.
  // After the `ref:`-string filter, the trait variant is TraitReference,
  // whose surface matches the override surface 1:1.
  if (built.traits && params.traitOverrides !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as TraitReference;
      if (typeof tr.ref !== "string" || typeof tr.name !== "string") return t;
      const overrides = params.traitOverrides as Record<string, _RefOverride | undefined> | undefined;
      const override = overrides?.[tr.name];
      if (!override) return t;
      const merged: TraitReference = { ...tr };
      if (override.config !== undefined) merged.config = { ...(tr.config ?? {}), ...override.config };
      if (override.events !== undefined) merged.events = { ...(tr.events ?? {}), ...override.events };
      if (override.effects !== undefined) merged.effects = { ...(tr.effects ?? {}), ...override.effects };
      if (override.listens !== undefined) merged.listens = override.listens;
      if (override.emitsScope !== undefined) merged.emitsScope = override.emitsScope;
      if (override.linkedEntity !== undefined) merged.linkedEntity = override.linkedEntity;
      return merged;
    });
  }
  // Zone 2: inline-trait state-machine splice, keyed by inline trait `name`.
  // After the `ref:`-not-set filter, the trait variant is Trait, whose
  // stateMachine block is what we splice into.
  if (built.traits && params.interactionStates !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as Trait & { ref?: string };
      if (typeof tr.ref === "string" || typeof tr.name !== "string") return t;
      const splices = params.interactionStates as Record<string, _InlineSplice | undefined> | undefined;
      const splice = splices?.[tr.name];
      if (!splice || !tr.stateMachine) return t;
      const sm = tr.stateMachine;
      const mergedSm: StateMachine = {
        ...sm,
        ...(splice.states !== undefined ? { states: [...(sm.states ?? []), ...splice.states] } : {}),
        ...(splice.events !== undefined ? { events: [...(sm.events ?? []), ...splice.events] } : {}),
        ...(splice.transitions !== undefined ? { transitions: [...(sm.transitions ?? []), ...splice.transitions] } : {}),
      };
      const mergedTrait: Trait = { ...tr, stateMachine: mergedSm };
      return mergedTrait;
    });
  }
  if (built.pages && params.pagePath !== undefined) {
    built.pages = (built.pages as _OrbPage[]).map((p, idx) => {
      if (!p || typeof p !== "object") return p;
      if (idx !== 0) return p;
      const out = { ...p } as _OrbPage & { path?: string };
      out.path = params.pagePath;
      return out;
    });
  }
  return built;
}

/** Phase 9 manifest — describes the params surface of stdLegalCaseBillableHourPanelOrbital. */
export const StdLegalCaseBillableHourPanelOrbitalManifest = {
  organism: 'std-legal-case',
  orbitalName: 'BillableHourPanelOrbital',
  paramFields: [
    { name: 'fields', type: 'EntityField[]', description: 'Extra fields appended to the canonical entity.' },
    { name: 'pagePath', type: 'string', description: 'URL override for the orbital first page.' },
    { name: 'persistence', type: "'persistent' | 'runtime' | 'singleton' | 'instance' | 'local'", description: 'Override the canonical entity persistence mode.' },
    { name: 'traitOverrides', type: 'Partial<Record<TraitName, MakeTraitRefOpts>>', description: 'Zone 1 — per-imported-trait override surface (config / events / effects / listens / emitsScope / linkedEntity). Atom owns topology.' },
    { name: 'interactionStates', type: 'Partial<Record<InlineTraitName, StateMachineSplice>>', description: 'Zone 2 — inline-trait state-machine splice (states / events / transitions). Factory owns topology.' },
  ] as const,
  traitNames: [
    'BillableHourAppLayout',
    'BillableHourTimesheetView',
  ] as const,
  inlineTraitNames: [
    'BillableHourPanel',
  ] as const,
};

/** Phase 9 typed guard — runtime validates StdLegalCaseBillableHourPanelOrbitalParams keys. */
export function isStdLegalCaseBillableHourPanelOrbitalParams(p: object): p is StdLegalCaseBillableHourPanelOrbitalParams {
  type _OverrideRecord = NonNullable<StdLegalCaseBillableHourPanelOrbitalParams['traitOverrides']>;
  type _SpliceRecord = NonNullable<StdLegalCaseBillableHourPanelOrbitalParams['interactionStates']>;
  const obj = p as { traitOverrides?: _OverrideRecord; interactionStates?: _SpliceRecord };
  if (obj.traitOverrides !== undefined) {
    if (typeof obj.traitOverrides !== "object" || obj.traitOverrides === null) return false;
    const allowed: readonly string[] = StdLegalCaseBillableHourPanelOrbitalManifest.traitNames;
    for (const k of Object.keys(obj.traitOverrides)) {
      if (!allowed.includes(k)) return false;
    }
  }
  if (obj.interactionStates !== undefined) {
    if (typeof obj.interactionStates !== "object" || obj.interactionStates === null) return false;
    const allowed: readonly string[] = StdLegalCaseBillableHourPanelOrbitalManifest.inlineTraitNames;
    for (const k of Object.keys(obj.interactionStates)) {
      if (!allowed.includes(k)) return false;
    }
  }
  return true;
}

/**
 * Tunable params for the CourtDeadlinePanelOrbital orbital.
 *
 * Canonical entity: CourtDeadline (locked — not overridable).
 * The factory hardcodes `linkedEntity` to the canonical entity on
 * every trait/page; renaming the entity would desync those references.
 *
 * Override surface (Phase 9):
 *   fields            — extra entity fields (appended)
 *   pagePath          — first-page URL override
 *   persistence       — entity persistence mode
 *   traitOverrides    — Zone 1: per-imported-trait override surface
 *                       (config / events / effects / listens / emitsScope /
 *                       linkedEntity). Atom owns topology.
 *   interactionStates — Zone 2: inline-trait state-machine splice
 *                       (states / events / transitions). Factory
 *                       owns topology.
 */
export interface StdLegalCaseCourtDeadlinePanelOrbitalParams {
  /** Extra fields appended to the canonical entity. */
  fields?: EntityField[];
  /** URL path override for the orbital's first page. */
  pagePath?: string;
  /** Override the canonical entity persistence mode. */
  persistence?: EntityPersistence;
  /**
   * Zone 1 — per-imported-trait override surface. Keyed on each
   * imported trait's local `name`. The atom owns state-machine
   * topology (states/events/transitions); these fields rewrite the
   * caller-side surface only.
   */
  traitOverrides?: Partial<Record<
    'CourtDeadlineAppLayout' | 'CourtDeadlineDocketView',
    Pick<MakeTraitRefOpts, 'config' | 'events' | 'effects' | 'listens' | 'emitsScope' | 'linkedEntity'>
  >>;
  /**
   * Zone 2 — inline-trait state-machine splice. Keyed on each inline
   * (factory-authored, no `ref:`) trait's `name`. Merges into the
   * matching inline trait's `stateMachine` block. Factory owns topology;
   * splice inserts caller-named states/events/transitions.
   */
  interactionStates?: Partial<Record<
    'CourtDeadlinePanel',
    Partial<Pick<NonNullable<Trait['stateMachine']>, 'states' | 'events' | 'transitions'>>
  >>;
}

/** Per-orbital factory: builds the CourtDeadlinePanelOrbital orbital with consumer params. */
export function stdLegalCaseCourtDeadlinePanelOrbital(params: StdLegalCaseCourtDeadlinePanelOrbitalParams = {}): OrbitalDefinition {
  const canonicalName = 'CourtDeadline';
  const built = makeOrbitalWithUses({
    name: 'CourtDeadlinePanelOrbital',
    uses: [
      {
        'from': 'std/behaviors/std-app-layout',
        'as': 'AppShell',
      },
      {
        'from': 'std/behaviors/std-court-deadline',
        'as': 'CourtDeadline',
      },
    ],
    entity: {
      name: canonicalName,
      collection: 'courtdeadlines',
      persistence: params.persistence ?? 'persistent',
      fields: [
        {
          'name': 'id',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'matterId',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'title',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'deadlineType',
          'type': 'string',
          'default': 'filing',
          'values': [
            'filing',
            'response',
            'discovery',
            'hearing',
            'trial',
            'other',
          ],
        },
        {
          'name': 'dueAt',
          'type': 'string',
          'required': true,
        },
        {
          'name': 'jurisdiction',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'assignedTo',
          'type': 'string',
          'default': '',
        },
        {
          'name': 'status',
          'type': 'string',
          'default': 'upcoming',
          'values': [
            'upcoming',
            'completed',
            'missed',
            'extended',
          ],
        },
        {
          'name': 'reminderSent',
          'type': 'boolean',
          'default': false,
        },
        {
          'name': 'notes',
          'type': 'string',
          'default': '',
        },
        ...(params.fields ?? []),
      ],
    } as Entity,
    traits: [
      makeTraitRef({
        'ref': 'AppShell.traits.AppLayout',
        'name': 'CourtDeadlineAppLayout',
        'config': {
          'notificationClickEvent': 'COURT_DEADLINE_NOTIFICATIONS_OPEN',
          'contentTrait': '@trait.CourtDeadlinePanel',
          'appName': 'Legal Case',
          'navItems': [
            {
              'label': 'Matters',
              'href': '/matters',
              'icon': 'scale',
            },
            {
              'icon': 'clock',
              'label': 'Time',
              'href': '/timesheet',
            },
            {
              'href': '/deadlines',
              'icon': 'calendar',
              'label': 'Deadlines',
            },
          ],
          'notifications': [],
          'searchEvent': 'COURT_DEADLINE_SEARCH',
        },
        'events': {
          'SEARCH': 'COURT_DEADLINE_SEARCH',
          'NOTIFY_CLICK': 'COURT_DEADLINE_NOTIFICATIONS_OPEN',
        },
      }),
      {
        'name': 'CourtDeadlinePanel',
        'category': 'interaction',
        'stateMachine': {
          'states': [
            {
              'name': 'composing',
              'isInitial': true,
            },
          ],
          'events': [
            {
              'key': 'INIT',
              'name': 'Initialize',
            },
          ],
          'transitions': [
            {
              'from': 'composing',
              'to': 'composing',
              'event': 'INIT',
              'effects': [
                [
                  'render-ui',
                  'main',
                  {
                    'gap': 'lg',
                    'direction': 'vertical',
                    'className': 'max-w-6xl mx-auto w-full p-4',
                    'children': [
                      {
                        'align': 'center',
                        'children': [
                          {
                            'type': 'icon',
                            'name': 'calendar',
                          },
                          {
                            'type': 'typography',
                            'variant': 'h2',
                            'content': 'Court Deadlines',
                          },
                        ],
                        'gap': 'sm',
                        'type': 'stack',
                        'direction': 'horizontal',
                      },
                      {
                        'type': 'divider',
                      },
                      '@trait.CourtDeadlineDocketView',
                    ],
                    'type': 'stack',
                  },
                ],
              ],
            },
          ],
        },
        'scope': 'instance',
      } as never,
      makeTraitRef({
        'ref': 'CourtDeadline.traits.CourtDeadlineDocket',
        'name': 'CourtDeadlineDocketView',
        'config': {
          'title': 'Filing Deadlines',
        },
      }),
    ],
    pages: [
      {
        'name': 'DeadlinesPage',
        'path': '/deadlines',
        'traits': [
          {
            'ref': 'CourtDeadlineAppLayout',
          },
          {
            'ref': 'CourtDeadlinePanel',
          },
          {
            'ref': 'CourtDeadlineDocketView',
          },
        ],
      } as never,
    ],
  });
  type _OrbTrait = OrbitalDefinition["traits"][number];
  type _OrbPage = NonNullable<OrbitalDefinition["pages"]>[number];
  type _RefOverride = Pick<MakeTraitRefOpts, "config" | "events" | "effects" | "listens" | "emitsScope" | "linkedEntity">;
  type _InlineSplice = Partial<Pick<NonNullable<Trait["stateMachine"]>, "states" | "events" | "transitions">>;
  // Zone 1: per-imported-trait override surface, keyed by trait `name`.
  // After the `ref:`-string filter, the trait variant is TraitReference,
  // whose surface matches the override surface 1:1.
  if (built.traits && params.traitOverrides !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as TraitReference;
      if (typeof tr.ref !== "string" || typeof tr.name !== "string") return t;
      const overrides = params.traitOverrides as Record<string, _RefOverride | undefined> | undefined;
      const override = overrides?.[tr.name];
      if (!override) return t;
      const merged: TraitReference = { ...tr };
      if (override.config !== undefined) merged.config = { ...(tr.config ?? {}), ...override.config };
      if (override.events !== undefined) merged.events = { ...(tr.events ?? {}), ...override.events };
      if (override.effects !== undefined) merged.effects = { ...(tr.effects ?? {}), ...override.effects };
      if (override.listens !== undefined) merged.listens = override.listens;
      if (override.emitsScope !== undefined) merged.emitsScope = override.emitsScope;
      if (override.linkedEntity !== undefined) merged.linkedEntity = override.linkedEntity;
      return merged;
    });
  }
  // Zone 2: inline-trait state-machine splice, keyed by inline trait `name`.
  // After the `ref:`-not-set filter, the trait variant is Trait, whose
  // stateMachine block is what we splice into.
  if (built.traits && params.interactionStates !== undefined) {
    built.traits = (built.traits as _OrbTrait[]).map((t): _OrbTrait => {
      if (!t || typeof t !== "object") return t;
      const tr = t as Trait & { ref?: string };
      if (typeof tr.ref === "string" || typeof tr.name !== "string") return t;
      const splices = params.interactionStates as Record<string, _InlineSplice | undefined> | undefined;
      const splice = splices?.[tr.name];
      if (!splice || !tr.stateMachine) return t;
      const sm = tr.stateMachine;
      const mergedSm: StateMachine = {
        ...sm,
        ...(splice.states !== undefined ? { states: [...(sm.states ?? []), ...splice.states] } : {}),
        ...(splice.events !== undefined ? { events: [...(sm.events ?? []), ...splice.events] } : {}),
        ...(splice.transitions !== undefined ? { transitions: [...(sm.transitions ?? []), ...splice.transitions] } : {}),
      };
      const mergedTrait: Trait = { ...tr, stateMachine: mergedSm };
      return mergedTrait;
    });
  }
  if (built.pages && params.pagePath !== undefined) {
    built.pages = (built.pages as _OrbPage[]).map((p, idx) => {
      if (!p || typeof p !== "object") return p;
      if (idx !== 0) return p;
      const out = { ...p } as _OrbPage & { path?: string };
      out.path = params.pagePath;
      return out;
    });
  }
  return built;
}

/** Phase 9 manifest — describes the params surface of stdLegalCaseCourtDeadlinePanelOrbital. */
export const StdLegalCaseCourtDeadlinePanelOrbitalManifest = {
  organism: 'std-legal-case',
  orbitalName: 'CourtDeadlinePanelOrbital',
  paramFields: [
    { name: 'fields', type: 'EntityField[]', description: 'Extra fields appended to the canonical entity.' },
    { name: 'pagePath', type: 'string', description: 'URL override for the orbital first page.' },
    { name: 'persistence', type: "'persistent' | 'runtime' | 'singleton' | 'instance' | 'local'", description: 'Override the canonical entity persistence mode.' },
    { name: 'traitOverrides', type: 'Partial<Record<TraitName, MakeTraitRefOpts>>', description: 'Zone 1 — per-imported-trait override surface (config / events / effects / listens / emitsScope / linkedEntity). Atom owns topology.' },
    { name: 'interactionStates', type: 'Partial<Record<InlineTraitName, StateMachineSplice>>', description: 'Zone 2 — inline-trait state-machine splice (states / events / transitions). Factory owns topology.' },
  ] as const,
  traitNames: [
    'CourtDeadlineAppLayout',
    'CourtDeadlineDocketView',
  ] as const,
  inlineTraitNames: [
    'CourtDeadlinePanel',
  ] as const,
};

/** Phase 9 typed guard — runtime validates StdLegalCaseCourtDeadlinePanelOrbitalParams keys. */
export function isStdLegalCaseCourtDeadlinePanelOrbitalParams(p: object): p is StdLegalCaseCourtDeadlinePanelOrbitalParams {
  type _OverrideRecord = NonNullable<StdLegalCaseCourtDeadlinePanelOrbitalParams['traitOverrides']>;
  type _SpliceRecord = NonNullable<StdLegalCaseCourtDeadlinePanelOrbitalParams['interactionStates']>;
  const obj = p as { traitOverrides?: _OverrideRecord; interactionStates?: _SpliceRecord };
  if (obj.traitOverrides !== undefined) {
    if (typeof obj.traitOverrides !== "object" || obj.traitOverrides === null) return false;
    const allowed: readonly string[] = StdLegalCaseCourtDeadlinePanelOrbitalManifest.traitNames;
    for (const k of Object.keys(obj.traitOverrides)) {
      if (!allowed.includes(k)) return false;
    }
  }
  if (obj.interactionStates !== undefined) {
    if (typeof obj.interactionStates !== "object" || obj.interactionStates === null) return false;
    const allowed: readonly string[] = StdLegalCaseCourtDeadlinePanelOrbitalManifest.inlineTraitNames;
    for (const k of Object.keys(obj.interactionStates)) {
      if (!allowed.includes(k)) return false;
    }
  }
  return true;
}

/**
 * Bundled params for std-legal-case — one optional entry per orbital.
 * Each entry maps to its per-orbital factory above.
 */
export interface StdLegalCaseParams {
  Matter?: StdLegalCaseMatterOrbitalParams;
  BillableHourPanel?: StdLegalCaseBillableHourPanelOrbitalParams;
  CourtDeadlinePanel?: StdLegalCaseCourtDeadlinePanelOrbitalParams;
}

/** Whole-organism descriptor (3 orbitals). Composes per-orbital factories. */
export function stdLegalCase(params: StdLegalCaseParams = {}): OrbitalDefinition[] {
  return [
    stdLegalCaseMatterOrbital(params.Matter ?? {}),
    stdLegalCaseBillableHourPanelOrbital(params.BillableHourPanel ?? {}),
    stdLegalCaseCourtDeadlinePanelOrbital(params.CourtDeadlinePanel ?? {}),
  ];
}
