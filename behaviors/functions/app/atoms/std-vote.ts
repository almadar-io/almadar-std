/**
 * std-vote — std-vote — forum-style up/down vote tally with single-vote-per-user constraint. Owns a Vote entity; VOTE persists the new direction (toggling to none when re-clicking the active arrow) and emits VoteCasted so cross-trait listeners can recompute aggregates.
 *
 ...
 *   node scripts/regenerate-std-ts.mjs --only=std-vote
 *
 * @level atom
 ...
 */
export type StdVoteEventKey = 'INIT' | 'VOTE' | 'VoteCastFailed' | 'VoteCasted' | 'VoteLoadFailed' | 'VoteLoaded';
