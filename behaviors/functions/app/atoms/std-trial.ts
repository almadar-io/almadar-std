/**
 * std-trial — std-trial — subscription trial countdown with Convert / Expire / Cancel actions. Listens for SET_STATUS so std-lifecycle can drive automated transitions. Exposes CRM-pattern knobs.
 *
 ...
 */
export type StdTrialEventKey = 'CANCEL' | 'CLOSE_VIEW' | 'CONVERT' | 'EXPIRE' | 'INIT' | 'OPEN_TRIAL' | 'SET_STATUS' | 'TrialLoadFailed' | 'TrialLoaded' | 'TrialUpdateFailed' | 'TrialUpdated';
...
/**
 * Payload shape for the `TrialUpdateFailed` event.
 */
export interface StdTrialTrialUpdateFailedPayload {
  error?: string;
  code?: string;
}
