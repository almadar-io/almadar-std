/**
 * std-lease — std-lease — owner-side lease records (tenant ↔ unit, dates, monthly rent, status). Listens for SET_STATUS so std-lifecycle can drive transitions (active → expired). Exposes Real-estate knobs (leaseRenewalNoticeDays, securityDepositMonths, lateRentGraceDays, …).
 *
 ...
 */
export type StdLeaseEventKey = 'CLOSE_VIEW' | 'INIT' | 'LeaseLoadFailed' | 'LeaseLoaded' | 'LeaseUpdateFailed' | 'LeaseUpdated' | 'OPEN_LEASE' | 'RENEW_LEASE' | 'SET_STATUS' | 'TERMINATE_LEASE';
...
/**
 * Payload shape for the `LeaseUpdated` event.
 */
export interface StdLeaseLeaseUpdatedPayload {
  row?: {
    id: string;
    tenantId: string;
    tenantName?: string;
...
/**
 * Payload shape for the `LeaseUpdateFailed` event.
 */
export interface StdLeaseLeaseUpdateFailedPayload {
  error?: string;
  code?: string;
}
