/**
 * std-help-article — std-help-article — public help center article (title, body, category, status, view count). Listens for SET_STATUS so std-lifecycle can drive transitions. Exposes Content/Knowledge knobs (visibilityScope, keepVersions, enabledLocales, …).
 *
 ...
 */
export type StdHelpArticleEventKey = 'ARCHIVE' | 'CLOSE_VIEW' | 'HelpArticleLoadFailed' | 'HelpArticleLoaded' | 'HelpArticleUpdateFailed' | 'HelpArticleUpdated' | 'INIT' | 'OPEN_ARTICLE' | 'PUBLISH' | 'SET_STATUS';
...
/**
 * Payload shape for the `HelpArticleUpdated` event.
 */
export interface StdHelpArticleHelpArticleUpdatedPayload {
  row?: {
    id: string;
    title: string;
    slug: string;
...
/**
 * Payload shape for the `HelpArticleUpdateFailed` event.
 */
export interface StdHelpArticleHelpArticleUpdateFailedPayload {
  message?: string;
}
