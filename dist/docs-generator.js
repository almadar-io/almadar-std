...
import { BEHAVIORS_BY_CATEGORY } from './behaviors/registry.js';
// ============================================================================
 ...
 */
export const BEHAVIOR_CATEGORY_DESCRIPTIONS = {
    'ui-interaction': {
        name: 'UI Interaction',
 ...
 */
export function humanizeOperatorName(operatorName) {
    ...
    if (specialNames[funcName])
        return specialNames[funcName];
 ...
 */
export function humanizeReturnType(returnType) {
    const typeMap = {
        number: 'number',
        string: 'text',
        boolean: 'true/false',
 ...
 */
export function formatArity(minArity, maxArity) {
    if (maxArity === null) {
 ...
 */
export function generateOperatorDoc(opName, meta) {
    return {
        name: opName,
 ...
 */
export function generateModuleDoc(moduleId) {
    const moduleInfo = MODULE_DESCRIPTIONS[moduleId];
    if (!moduleInfo)
        return null;
    const moduleOperators = STD_OPERATORS_BY_MODULE[moduleId] || {};
    const operators = Object.entries(moduleOperators).map(([opName, meta]) => generateOperatorDoc(opName, meta));
    return {
        id: moduleId,
        ...moduleInfo,
        operators,
 ...
 */
export function generateBehaviorDoc(behavior) {
 ...
 */
function generateBehaviorSourceCode(behavior) {
    const lines = [];
    const indent = '  ';
    ...
    if (behavior.dataEntities && behavior.dataEntities.length > 0) {
        ...
        for (const entity of behavior.dataEntities) {
            ...
            if (entity.runtime)
                lines.push(`${indent}${indent}${indent}runtime: true,`);
            if (entity.singleton)
                lines.push(`${indent}${indent}${indent}singleton: true,`);
    ...
    if (behavior.stateMachine) {
        ...
        for (const t of sm.transitions) {
            ...
            if (t.to)
                lines.push(`${indent}${indent}${indent}${indent}to: '${t.to}',`);
    ...
    if (behavior.ticks && behavior.ticks.length > 0) {
        ...
        for (const tick of behavior.ticks) {
            ...
            if (tick.priority)
                lines.push(`${indent}${indent}${indent}priority: ${tick.priority},`);
            if (tick.guard) {
 ...
 */
function formatSExprForCode(expr) {
 ...
 */
export function generateModulesDocs() {
    const modules = [];
 ...
 */
export function generateBehaviorsDocs() {
    const categories = [];
    // Group behaviors by category
    for (const [categoryId, categoryInfo] of Object.entries(BEHAVIOR_CATEGORY_DESCRIPTIONS)) {
        const categoryBehaviors = BEHAVIORS_BY_CATEGORY[categoryId] || [];
