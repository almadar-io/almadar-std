 ...
 */
export function isActionValidForComponent(action, component) {
 ...
 */
export function isActionInvalidForComponent(action, component) {
    const affinity = ACTION_AFFINITY[component];
    if (!affinity) {
        return false;
    }
 ...
 */
export function getValidActionsForComponent(component) {
    const affinity = ACTION_AFFINITY[component];
 ...
 */
export function getInvalidActionsForComponent(component) {
    const affinity = ACTION_AFFINITY[component];
 ...
 */
export function getComponentsForEvent(event) {
 ...
 */
export function validateActionsForComponent(actions, component) {
    const errors = [];
    const affinity = ACTION_AFFINITY[component];
    if (!affinity) {
    ...
    for (const action of actions) {
        if (action.event && affinity.invalid.includes(action.event)) {
            errors.push(`Action "${action.event}" is not valid on "${component}". ` +
 ...
 */
export function getAllKnownComponents() {
