...
import { validateTraitBindings, validateAllBehaviorBindings, formatSExprErrors, } from '../../validation/sexpr-validator.js';
import { validateOrbitalSchema, } from '../../validation/index.js';
import { getAllBehaviors, getAllBehaviorNames, BEHAVIORS_BY_CATEGORY, } from '../behaviors/index.js';
// ============================================================================
// Helper: Convert StandardBehavior to TraitDefinition for OrbitalSchema
// ============================================================================
function behaviorToTraitDefinition(behavior) {
    return {
        name: behavior.name,
        category: behavior.category?.startsWith('game') ? 'interaction' : 'interaction',
        stateMachine: {
            states: behavior.stateMachine?.states?.map(s => typeof s === 'string' ? { name: s, isInitial: s === behavior.stateMachine?.initial } : s) ?? [],
 ...
 */
function createTestSchema(behavior) {
    // Create entity fields from behavior's dataEntities
    const entityFields = behavior.dataEntities?.flatMap(de => de.fields?.map(f => ({
        name: f.name,
        type: f.type,
        required: false,
        default: f.default,
...
describe('std/* Behaviors Validation', () => {
    ...
    describe('All behaviors have valid bindings', () => {
        ...
        it.each(allBehaviorNames)('validates %s', (behaviorName) => {
            ...
            if (!result.valid) {
                ...
                const errorDetails = result.errors.map(e => `  - ${e.context?.binding ?? 'unknown'}: ${e.context?.message ?? e.message}`).join('\n');
                throw new Error(`Behavior "${behaviorName}" has invalid bindings:\n${errorDetails}\n\n` +
    ...
    describe('Batch validation', () => {
        it('validates all behaviors at once', () => {
            ...
            if (!result.valid) {
                const errorSummary = result.errors.slice(0, 10).map(e => `  - [${e.context?.traitName}] ${e.context?.binding}: ${e.context?.root}`).join('\n');
                throw new Error(`Found ${result.errors.length} invalid binding(s) in std/* behaviors:\n${errorSummary}`);
            }
            expect(result.valid).toBe(true);
        });
    ...
    describe('Category-specific validation', () => {
        const categories = Object.keys(BEHAVIORS_BY_CATEGORY);
        it.each(categories)('validates all %s behaviors', (category) => {
            ...
            if (!result.valid) {
                const errorSummary = result.errors.map(e => `  - [${e.context?.traitName}] ${e.context?.binding}`).join('\n');
                throw new Error(`Category "${category}" has behaviors with invalid bindings:\n${errorSummary}`);
            }
            expect(result.valid).toBe(true);
        });
    });
    describe('Specific behavior patterns', () => {
        ...
        it('validates game behaviors that use @config', () => {
            ...
            for (const behavior of gameBehaviors) {
                const result = validateTraitBindings(behavior);
                if (!result.valid) {
                    throw new Error(`Game behavior "${behavior.name}" has invalid bindings:\n` +
                        formatSExprErrors(result));
                }
                expect(result.valid).toBe(true);
            }
            ...
            for (const behavior of allBehaviors) {
                ...
                for (const pattern of invalidPatterns) {
                    if (json.includes(pattern)) {
                        throw new Error(`Behavior "${behavior.name}" contains invalid binding pattern "${pattern}"`);
                    }
                }
            }
        });
    ...
    describe('Full OrbitalSchema validation', () => {
        it.each(allBehaviorNames)('validates %s in OrbitalSchema context', (behaviorName) => {
            ...
            if (!result.valid) {
                ...
                const criticalErrors = result.errors.filter(e => !e.code.includes('INIT') && !e.code.includes('ORPHAN'));
                if (criticalErrors.length > 0) {
                    const errorDetails = criticalErrors.map(e => `  - [${e.code}] ${e.message} at ${e.path}`).join('\n');
                    throw new Error(`Behavior "${behaviorName}" failed OrbitalSchema validation:\n${errorDetails}`);
                }
            }
        ...
        it('validates all game behaviors as a combined schema', () => {
                    ...
                    entity: {
                        ...
                        persistence: behavior.dataEntities?.[0]?.runtime
                            ...
                            : behavior.dataEntities?.[0]?.singleton
                                ? { singleton: true }
                                : { collection: 'test_collection' },
                        fields: behavior.dataEntities?.flatMap(de => de.fields?.map(f => ({
                            name: f.name,
                            type: f.type,
                            required: false,
                            default: f.default,
            ...
            const criticalErrors = result.errors.filter(e => !e.code.includes('INIT') && !e.code.includes('ORPHAN') && !e.code.includes('DUPLICATE'));
            if (criticalErrors.length > 0) {
                const errorSummary = criticalErrors.slice(0, 5).map(e => `  - [${e.code}] ${e.message}`).join('\n');
                throw new Error(`Combined game schema has ${criticalErrors.length} critical error(s):\n${errorSummary}`);
            }
